...
print(snake_nest[1] is snake_nest[2] is snake_nest[3] is snake_nest[4] is snake_nest[0])
print(snake_nest[1] is snake_nest[2] is snake_nest[3] is snake_nest[4])
...
print(all(len(w) > 4 for w in sent))  # all()检查全部满足条件
print(any(len(w) > 4 for w in sent))  # any()检查部分满足条件
...
print(list(zip(words, tags, tags)))
...
print(words[0] == words[7])  # 句子开头和结尾的双引号是有区别的
...
print(list(nltk.trigrams(sent)))
print(list(nltk.ngrams(sent, 4)))
...
print(freq_words(consitution, 30))
...
print(list(permutations(['police'])))
print(list(permutations(['police', 'fish'])))
print(list(permutations(['police', 'fish', 'buffalo'])))
print(list(permutations(['police', 'fish', 'buffalo', 'cowboy'])))
print(list(permutations(['police', 'fish', 'buffalo', 'cowboy', 'gun'])))
print(list(permutations(['police', 'fish', 'buffalo', 'cowboy', 'gun', 'field'])))
...
# list()函数不支持将函数作为参数传入
# print(list(len, ['a', 'b', 'c']))
print(sum(lengths) / len(lengths))  # 布朗语料库中句子的平均长度
lengths = [len(w) for w in nltk.corpus.brown.sents(categories='news')]
print(sum(lengths) / len(lengths))
# 多重函数嵌套
print(list(filter(lambda c: c.lower() in "aeiou", 'their')))
...
print([[c for c in w if c.lower() in "aeiou"] for w in sent])

print([c for w in sent for c in w if c.lower() in "aeiou"])
...
print(list(zip(*song)))  # *song 表示分解输入数据，等价于song[0],song[1],song[2],song[3]，分组的数目就是输入的数目
print(list(zip(song)))


# 增加 verbose 参数，可以设置为调试开关
def freq_words(file, min=1, num=10, verbose=False):
    ...
    for word in nltk.word_tokenize(text):
        if len(word) >= min:
            freqdist[word] += 1
            if verbose and freqdist.N() % 100 != 0: print('.', end=' ')
    if verbose: print()
    return freqdist.most_common(num)


print(freq_words('output.txt', 4, 10, True))
...
print(find_words(['omg', 'teh', 'lolcat', 'sitted', 'on', 'teh', 'mat'], 3))
print(find_words(['omg', 'teh', 'lolcat', 'sitted', 'on', 'teh', 'mat'], 2))
