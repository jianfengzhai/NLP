...
document = open('PythonNLP/dict.files/dict.htm').read()
...
writer = csv.writer(open('PythonNLP/dict.files/dict1.csv', 'w', encoding = 'utf-8'))
writer.writerows(lexical_data('PythonNLP/dict.files/dict.htm', encoding = 'GB2312'))

# 3.3 从电子表格和数据库中获取数据
import csv

lexicon = csv.reader(open('PythonNLP/dict.files/dict.csv'))
...
idx = nltk.Index((defn_word, lexeme)
                 ...
                 if len(defn_word) > 3)
with open('PythonNLP/dict.files/dict.idx', 'w') as idx_file:
