...
print(model1.satisfiers(fmla1, 'x', grammar1))

# 前面为真，后面也为真，公式就是真；前面为假，后面无论什么，公式都是真
fmla2 = read_expr('girl(x)->walk(x)')
print(model1.satisfiers(fmla2, 'x', grammar1))

fmla3 = read_expr('walk(x)->girl(x)')
print(model1.satisfiers(fmla3, 'x', grammar1))

print(model1.evaluate('all x.(girl(x) -> walk(x))', grammar1))
...
print(model2.satisfiers(fmla4, 'x', grammar2))

fmla5 = read_expr('(person(y) & all x.(person(x) -> admire(x,y)))')
print(model2.satisfiers(fmla5, 'y', grammar2))

fmla6 = read_expr('(person(y) & all x.((x=bruce | x=julia) -> admire(x,y)))')
print(model2.satisfiers(fmla6, 'y', grammar2))
